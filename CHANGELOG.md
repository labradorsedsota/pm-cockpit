...
### Added
- startup-checklist.md：新增步骤 7「创建巡检 cron（硬卡点）」— 包含创建命令 + 验证命令 + 通过/失败判定，验证不通过不允许继续
- 步骤 8 启动公告新增巡检已开启的说明

### 来源
Phase 0 巡检未生效（心跳间隔 1 小时做不到 15 分钟催促）+ OMS 项目切换时未联动创建新 cron（规范写了但没执行）。根因：缺乏创建后的闭环验证步骤。
