...
> v1.5 更新：2026-04-10 | 变更：新增条款 1c（数据隔离保障，衔接设计规范条款 D2）、新增文档定位与链条引用
> v1.6 更新：2026-04-10 | 变更：条款 1b 新增 DevTools Console 降级链（OMS 回顾）、条款 6a/7a 新增 BLOCKED 强制诊断要求（OMS 回顾）

---

## 文档定位

本文档是测试规范三层链条的**第三层（执行层）**，解决"如何正确执行测试"。

| 层级 | 文档 | 职责 |
|------|------|------|
| 指派层 | [test-requirements.md](../skill/references/test-requirements.md) | PM 如何指派测试任务 |
| 设计层 | [test-case-design-spec.md](./test-case-design-spec.md) | QA 如何设计测试用例 |
| **执行层** | **本文档** | **QA 如何正确执行测试** |

**工作流：** PM 指派（指派层）→ QA 设计用例（设计层）→ QA 执行测试（本文档）
...
| 测试间数据状态污染 | 写操作用例破坏后续读操作用例的数据依赖 | 条款 1c（v1.5） |
| AppleScript JS 权限不可用时无降级方案 | L3.2/L3.3/L3.6 直接 BLOCKED | 条款 1b 降级链（v1.6） |
| BLOCKED 标记后无诊断、无方案 | L3.2/L3.3/L3.6 悬空 16 小时 | 条款 6a/7a 强制诊断（v1.6） |
...
### 条款 1b-fallback：Pre-flight JS 操作降级链（v1.6 新增）

> 来源：OMS 项目回顾 — Chrome AppleScript `execute javascript` 权限关闭导致 L3.2/L3.3/L3.6 BLOCKED

当条款 1b 的 AppleScript JS 注入失败（权限被关闭、沙箱限制等）时，按以下降级链依次尝试：

| 优先级 | 方式 | 适用场景 |
|--------|------|---------|
| 1（首选） | AppleScript `execute javascript` | 权限开启时 |
| 2（降级） | AppleScript 打开 DevTools Console + 注入脚本 | JS 权限关闭时 |
| 3（兜底） | 标记 BLOCKED + 强制诊断（条款 6a/7a） | 两者均不可用 |

**DevTools Console 降级执行方式：**
```bash
...
**使用边界（强制）：**
- DevTools Console **仅用于 Pre-flight 数据操作**（localStorage 读写、DOM 状态预置）
- **判定阶段必须走 mano-cua GUI 观测**，不可用 Console 查询替代视觉验证
- 使用 Console 降级时，TEST-REPORT 中该测试点须标注 `Pre-flight: DevTools Console（降级）`

**隔离保证：** 每条测试的预处理注入内容在条款 14（关闭标签页）+ 条款 1（重新加载页面）后自动清除，不影响下一条测试。后端应用需额外在 Pre-flight 中清理数据库状态。

### 条款 1c：数据隔离保障（v1.5 新增）

本条款与 [test-case-design-spec.md 条款 D2](./test-case-design-spec.md#条款-d2数据隔离声明) 双向衔接。设计规范负责声明数据依赖，本条款负责执行清理。

**执行时序：** 条款 1c 在条款 1（页面重置）之前执行，即 Pre-flight 的第一步。

**执行流程：**

1. 读取当前测试用例的 `数据依赖` 字段（由设计规范条款 D2 定义）
2. 根据依赖类型执行对应清理：

| 数据依赖 | 清理动作 |
|----------|----------|
| `SEED` | 清空 localStorage → 重新加载页面（触发应用种子数据注入） |
| `EMPTY` | 清空 localStorage → 注入空数组阻止种子数据注入 → 重新加载页面 |
| `CUSTOM` | 清空 localStorage → 按用例指定的脚本注入自定义数据 → 重新加载页面 |
| `ANY` | 仅执行标准条款 1（页面重置），不额外清理 |

3. 清理完成后继续执行条款 1 → 1a → 1b → 2 的正常流程

**后端应用适配：** 对于后端应用（如数据库驱动），`SEED`/`EMPTY`/`CUSTOM` 的清理动作替换为对应的 API 调用或 SQL 重置。具体脚本由 QA 在设计用例时在 `Pre-flight` 部分提供。

**强制要求：** 当用例的冲突标记（设计规范条款 D2.4）标注了 ⚠ 冒号时，本条款的清理步骤不可跳过，即使数据依赖为 `ANY`。
...
### 条款 6a/7a：重试上限 + BLOCKED 强制诊断（v1.1 新增，v1.6 修订）

条款 6、7 的终止重启统一上限 3 次。连续 3 次失败后，标记该测试点为 BLOCKED。

**BLOCKED 强制诊断（v1.6 新增）：**

> 来源：OMS 项目回顾 — 3 条测试标 BLOCKED 后无诊断，悬空 16 小时

标记 BLOCKED 时，**禁止只标状态跳过**。必须同时输出：
...
BLOCKED 测试点进入最终报告的异常清单，计入覆盖率统计时标记为"未执行"。诊断信息一并写入异常清单。
...
[ ] BLOCKED → 已附诊断（现象+根因+≥2方案）（条款 6a/7a v1.6）
[ ] BLOCKED 测试点已记入异常清单
```
