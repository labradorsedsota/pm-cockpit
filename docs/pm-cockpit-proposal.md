...
5. **和品鉴者确认群聊结构**
   - 所有环节在一个群，还是按环节分群？
   - 如果分群：每个群对应哪个环节、哪些人/Bot 在哪个群
   - 如果单群：是否需要用消息前缀区分环节（如 `[测试]`、`[研发]`）
   - 确认结果记入项目元信息
6. **指定人类品鉴者**
   - 巡检升级时通知谁（一人或多人）
   - 写入项目元信息
7. **创建应用注册表**（`config/apps.yaml`）
8. **拆解任务并创建 Issues**
...
### 5.6 任务上下文快照（解决跨 session 连续性）

**问题：** Bot session 重启后上下文丢失，接手的 session 不知道之前做到哪了。

**触发：** 任务暂停、session 即将结束、或 Bot 预感 session 可能中断时。
...
**当前状态：** {In Progress / Blocked / On Hold}
**进度：** {做到哪一步了，具体描述}
**下一步：** {接手后第一件要做的事}
**阻塞项：** {如果有的话}
**关键文件：**
...
**相关群 ID：** {群 channel_id}
**待确认事项：** {需要和谁确认什么}
**上下文备注：** {任何接手者需要知道的隐含信息}
```

**恢复流程：** 新 session 启动时，读取 memory 中最新的任务快照，从"下一步"继续。

**PM 责任：** 巡检时检查活跃任务是否有最新快照。任务 Owner 是 Bot 且超过 2 小时未更新快照 → 提醒写入。
...
│   ├── snapshot-template.md         # 任务上下文快照模板
